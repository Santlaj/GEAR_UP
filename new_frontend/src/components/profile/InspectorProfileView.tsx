...
export const InspectorProfileView: React.FC<InspectorProfileViewProps> = ({
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    ...
    try {
      // 1. Fetch authoritative user profile from /api/auth/me
      ...
      if (profileResult.status === 'fulfilled') {
        ...
        console.warn('Profile fetch from /api/auth/me failed:', profileResult.reason);
        if (!initialUser) {
          setError('Unable to load authoritative officer profile from /api/auth/me');
        }
      }
    ...
    return (
      <div className="w-full px-4 sm:px-6 py-16 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[400px] text-center">
        ...
        <p className="text-xs text-slate-500 mt-1">
          Querying central authority endpoint: <code className="font-mono text-slate-700 font-semibold">GET /api/auth/me</code>
        </p>
      </div>
    );
  }
