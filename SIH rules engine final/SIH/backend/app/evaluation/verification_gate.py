...
# Status values that allow authoritative legal results
...
def apply_verification_gate(
    ...
    Args:
        raw_result: The evaluator's raw result (PASS, FAIL, UNCERTAIN).
        verification_status: The rule's verification status.
        mode: Execution mode — "production" or "demo".

    Returns:
        (final_status, authoritative, reason) where:
        - final_status: COMPLIANT, NON_COMPLIANT, or NEEDS_REVIEW
        - authoritative: True only for VERIFIED rules in production mode
        - reason: Explanation when the gate overrides or annotates the result
    """
    ...
    if is_verified:
        # Verified rule — use raw evaluator result as authoritative
    ...
    if mode == "demo":
        # Demo mode: expose the raw evaluator result for demonstration
        # but explicitly mark as non-authoritative
        if raw_result == RawResult.PASS:
            status = "COMPLIANT"
        elif raw_result == RawResult.FAIL:
