...
# ---------------------------------------------------------------------------
# Scope gates (derived from the source documents)
# ---------------------------------------------------------------------------
...
def _gate_rule_3a_bulk_package(ctx: ScopeContext) -> ScopeGateResult:
    ...
    except cement and fertilizer sold in bags up to 50 kg.
    """
    ...
    # Convert to canonical units for comparison
    ...
    if weight_kg is not None and weight_kg > 25:
        # Exception: cement and fertilizer up to 50 kg
...
def _gate_rule_3b_industrial_institutional(ctx: ScopeContext) -> ScopeGateResult:
    """Rule 3(b): Chapter II does not apply to packages meant for
...
def _gate_rule_26_drugs(ctx: ScopeContext) -> ScopeGateResult:
    """Rule 26: Exemption for scheduled/non-scheduled drug formulations."""
...
def _gate_rule_26_agricultural_produce(ctx: ScopeContext) -> ScopeGateResult:
    """Rule 26: Exemption for agricultural produce packages > 50 kg."""
...
def evaluate_scope(ctx: ScopeContext) -> ScopeEvaluationResult:
    ...
    Returns the aggregate scope evaluation with per-gate results.
    """
