...
def _normalize_for_comparison(value: str) -> str:
    ...
    This is used only to detect agreement between differently-formatted values.
    """
    v = value.strip().lower()
    # Currency symbols
...
def _resolve_field(
    ...
    Mutation: modifies field in-place and returns it.
    """
    ...
    # Disagreement — try to resolve by weighted scoring
    # Score = confidence * source_reliability
    ...
    # If the gap between best and second is large enough, we can resolve
    gap_threshold = 0.15
    ...
    # Cannot safely resolve — mark as CONFLICT
    field.resolved_value = best_candidate.value  # tentative, but flagged
    field.confidence = best_candidate.confidence
    field.bbox = best_candidate.bbox
    field.source = best_candidate.source
...
def resolve_conflicts(
    ...
    This is a pipeline stage that must be called before rule evaluation.
    Modifies the extraction output in-place and returns it.
    """
