...
def map_domains(
    ...
    Args:
        classification: The commodity classification result.
        is_packaged: Whether the product is in packaged form (default True).
        is_pre_packaged_food: Explicit override for FSSAI pre-packaged food status.

    Returns:
        DomainMappingResult with applicability for each domain.
    """
    result = DomainMappingResult(food_classification=classification.classification)

    # All domains are candidates initially
    result.candidate_domains = ["legal_metrology", "fssai"]

    # ---- Legal Metrology applicability --------------------------------------
    # LM applies to all packaged commodities (food and non-food)
    if is_packaged:
